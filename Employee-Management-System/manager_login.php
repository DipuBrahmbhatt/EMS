...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    $emailInput = trim($_POST['email']); // Using email instead of username
    $passwordInput = trim($_POST['password']);

    // Check if the email exists and the user has 'manager' role
    $stmt = $conn->prepare("SELECT * FROM users WHERE email = ? AND role = 'manager'");
    $stmt->bind_param("s", $emailInput); // Bind the email
    $stmt->execute();
    ...
    if ($result && $result->num_rows === 1) {
        ...
        if ($passwordInput === $user['password']) {
            ...
            echo "<script>alert('Manager login successful!'); window.location.href='manager_dashhboard.php';</script>";
            exit;
        } else {
            $loginMessage = "<div class='error'>Invalid password.</div>";
        }
    } else {
        $loginMessage = "<div class='error'>Invalid email or not authorized as Manager.</div>";
    }
