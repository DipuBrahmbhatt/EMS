...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Check if the email exists and the user has 'hr' role
    $stmt = $conn->prepare("SELECT * FROM users WHERE email = ? AND role = 'hr'");
    ...
    if ($result && $result->num_rows === 1) {
        ...
        if ($passwordInput === $user['password']) {
            ...
            echo "<script>alert('HR login successful!'); window.location.href='hr_dashboard.php';</script>";
            exit;
        } else {
            $loginMessage = "<div class='error'>Invalid password.</div>";
        }
    } else {
        $loginMessage = "<div class='error'>Invalid email or not authorized as HR.</div>";
    }
